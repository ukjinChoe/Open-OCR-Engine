...
from utils.utils import decode_image_from_string, sort_wordBoxes, get_vocab
...
app = Flask(__name__)
__VALID_KEY = "OPEN-OCR"

def get_ocr_result(image):
    # text detection
    # input_image, resize_info = craft.preprocessImage(image)
    # output = craft(input_image)
    # boxes = craft.get_boxes(output, resize_info)
    
    image_tensor, original_size = craft.preprocessImage(image)

    output = craft(image_tensor)
    
    boxes = craft.get_boxes(output, original_size)
    ...
    text_for_pred = torch.LongTensor(batch.size(0), cfg.batch_max_length + 1)
...
def auth(func):
    def wrapper(*args, **kwargs):
        if request.method == 'POST':
            ...
            if 'key' in req and req['key'] == __VALID_KEY:
